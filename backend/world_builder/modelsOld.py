...
class SpineNode(BaseModel):
    position: Vec2 = Field(description="Position of this node on the spine.")
...
class SpineGeometry(BaseModel):
    ...
    nodes: List[SpineNode] = Field(description="Nodes defining the spine and its local volume.")

class CoordinateUpdateModel(BaseModel):
    positions: Dict[str, Vec2] = Field(default_factory=dict, description="Label positions.")
    geometries: Dict[str, Union[CircleGeometry, SpineGeometry]] = Field(default_factory=dict)
